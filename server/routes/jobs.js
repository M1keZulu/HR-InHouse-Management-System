...
router.post("/addJob", validateToken, (req, res) => {
    const { title, description, extra_requirements, salary, company_id } = req.body;
    connection.query(
        "INSERT INTO jobs (title, description, extra_requirements, salary, company_id) VALUES (?, ?, ?, ?, ?)",
        [title, description, extra_requirements, salary, company_id],
        (error, results) => {
            if (error) {
                console.log(error);
                res.json({
